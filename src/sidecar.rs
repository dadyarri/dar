...
fn compute_blake3_hex(volume_path: &Path) -> Result<String> {
    let mut file = File::open(volume_path).wrap_err_with(|| {
        t!(
    ...
    loop {
        let read = file.read(&mut buf).wrap_err_with(|| {
            t!(
...
pub fn write_b3_sidecar(volume_path: &Path) -> Result<()> {
    ...
    let mut out = File::create(&sidecar_path).wrap_err_with(|| {
        t!(
    ...
    writeln!(out, "{digest}  {file_name}").wrap_err_with(|| {
        t!(
...
pub fn verify_b3_sidecar(volume_path: &Path) -> Result<bool> {
    let sidecar_path = sidecar_path_for(volume_path);
    let file = File::open(&sidecar_path).wrap_err_with(|| {
        t!(
    ...
    reader.read_line(&mut line).wrap_err_with(|| {
        t!(
